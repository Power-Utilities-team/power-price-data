...
| 8 | The Windows Open XML validation job runs AFTER the build job has already committed, so an invalid package can land in `deliverables/` and only then fail the run — which is exactly what happened on 2026-07-31 with the frozen workbook. Move the commit step behind the validate job, or gate it on validation passing | OPEN | Nothing — but restructure it between runs, not while one is in flight |
